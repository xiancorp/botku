...
function create_response($text, $message)
{
    ...
    switch ($command[0]) {
		...
		case '/start':
            $hasil = "hai..........`$namauser`, selamat datang, di bot inventori, silahkan melakukan pendaftaran untuk menggunakan menu.\nuntuk bantuan ketik /help.\nkemudian untuk mendaftar ketik /daftar.";
            break;
				...
				if(isset($_SESSION['step'])){
					switch($_SESSION['step']){
						case 'nama':
							...
							$_SESSION['nama'] = $text;
							$hasil = "Ok, {$text}.\nKirimkan nomer telepon yang bisa dihubungi!";
							$_SESSION['chat_id'] = $chatid;
						...
						case 'email':
							$_SESSION['email'] = $text;
							$hasil = "Nama:\t {$_SESSION['nama']},\ntelp:\t{$_SESSION['telepon']},\nemail:\t {$_SESSION['email']}.\nApakah sudah benar? (ya/tidak)";
							$_SESSION['chat_id'] = $chatid;
							$_SESSION['step'] = 'verifikasi';
							break;
						case 'verifikasi':
							if($text == 'ya'){
								$hasil = "Terima kasih sudah bergabung di Klub Belajar Linux (KBL) Bumigora STMIK Bumigora Mataram. Silahkan bergabung";
						...
						case 'selesai':
							// simpan ke database
                        // $insert=insert("INSERT INTO {$this->tabel} VALUES('{$data['id']}','{$data['nama']}','{$data['telepon']}','{$data['email']}')");
							// $this->insert($data);
							session_destroy();
							break;
					...
					$hasil = "Hi, Saya Bot yang akan membantumu mendaftar,\nsilahkan klik /daftar";
				}
			// }
            break;
